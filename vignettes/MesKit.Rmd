...
`Hugo_Symbol`, `Chromosome`, `Start_Position`, `End_Position`, `Variant_Classification`, `Variant_Type`, `Reference_Allele`, `Tumor_Seq_Allele2`, `Ref_allele_depth`, `Alt_allele_dept`, `VAF`, `Tumor_Sample_Barcode`
   
   
**Note:** Values in the `Hugo_Symbol` field are not necessarily from the HUGO database.
...
Clinical data file contains clinical information about each patient and their tumor samples, and mandatory fields are `Tumor_Sample_Barcode`, `Tumor_ID`, `Patient_ID`, and `Tumor_Label`.

**Example clinical data file**

```{r echo=FALSE, paged.print=FALSE, rownames.print=FALSE}
ClinInfo <- read.table(system.file("extdata", "HCC_LDC.clin.txt", package = "MesKit"), header = TRUE)
ClinInfo[1:5, ]
```  
...
* `Tumor_Label` - the specific label of each tumor sample
...
ccf.pairwise <- ccf.list$HCC8257[["HCC8257_T3-HCC8257_T4"]]
...
graphics::smoothScatter(matrix(c(ccf.pairwise[,3], ccf.pairwise[,4]),ncol = 2),
              ...
              xlab = "HCC8257_T3", ylab = "HCC8257_T4")
## show driver genes
...
neutralResult$model.fitting.plot$HCC8257_T3
```     
