...
seg <- readSegment(segCN.file = segCN, 
...
Distinct patterns of monoclonal versus polyclonal seeding based on the cancer cell fraction (CCF) of SSNVs between sample pairs. ```ccfDensity``` function prints pairwise CCF plots of mutations which are identified across samples from a single patient. Recent studies have used this method to deduce the potential metastatic route between different paired tumor lesions.(Quantitative evidence for early metastatic seeding in colorectal cancer)(Genomic and Transcriptomic Profiling of Combined Hepatocellular and Intrahepatic Cholangiocarcinoma Reveals Distinct Molecular Subtypes)  
...
Nei’s genetic distance is used in population genetics to assess the similarity between populations, taking into account heterogeneity within populations. The function ```calNeiDist``` calculates Nei's distance of CCF among multi-region samples from the sanme patient. for each sample-pair.(Whole-genome sequencing identifies recurrent mutations in hepatocellular carcinoma)
...
#### 8.2 Compare Phylogenetic tree
Comparison between phylogenetic trees can reveal concensus patterns of tumor evolution. The ```compareeTree``` function computes distances between phylogenetic trees constructed through different methods. This function returns a vector containing four distance methods by ```treedist``` from ```phangorn``` R package. See treedist for details(http://evolution.genetics.washington.edu/phylip/doc/treedist.html).
...
#### 8.5 Pathway enrichment analysis
```{r, message=FALSE, warning=FALSE}
...
#### 8.6 Mutational signature analysis
