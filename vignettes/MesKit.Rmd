...
```{r message=FALSE, fig.align='left', fig.width=8.5, fig.height=5}
...
#### 6.2 AUC of CCF
The ```ccfAUC``` function estimates the tumor heterogeneity via the area under the curve (AUC) of the cumulative density from all cancer cell fractions per tumor.  Tumors were considered homogenous within the lower 
quartile of the AUC distribution whereas all other tumors were considered heterogeneous.   
In the results, tumor sample attached with a ```*``` on the top right indicates that, comparing with homogenous samples, it is heterogeneous.
   
```{r message=FALSE, fig.height=4.5, fig.width=5}
ccfAUC(maf, auc.cutoff = 0.145)
```  

### 7. Inter-tumor heterogeneity evaluation
...
Parameters ```minVaf``` and ```maxVaf``` can be set to filter mutations. In addition, we offer several options for defining the plotting styles. Parameter `plotOption` could be set as ```"separate"```, ```"combine"``` or ```"compare"```, which means to generate a frequency map in different documents.  
...
#### 7.2 Private/Shared mutation identification
...
Fixation Index (Fst) estimation is a classical metric of population genetics, which can be used to measures the genetic divergence between regions[ Weir, B. S. & Cockerham, C. C. Estimating F-statistics for the analysis of 
population-structure][Estimating and interpreting FST: the impact of rare variants. Genome research][ Genetics in geographically structured populations: defining, estimating and interpreting F(ST)]. ```calFst``` function calculates the proportion of the total variance in allele frequency caused by frequency differences between populations.
...
```calJSI``` function calculates the Jaccard Similarity Index (JSI) for sample pairs from a single patient. JSI is defined for the intersection divided by the union of these two sets. Higher JSI values are more likely indicates polyclonal seeding [1](#References). 
  
```{r fig.align='left', fig.width=5.5, fig.height=5.5, fig.asp=1.0}
calJSI(maf, type = "lower", plot = TRUE)
```  
...
```{r, warning = FALSE, message=FALSE}
treeMutSig <- treeMutationalSig(phylotree, driverGenesFile = driverGene.file, mutThreshold = 50)
```
...
```{r, warning = FALSE, message=FALSE}
mutTrunkBranch(treeMutSig, conf.level = 0.95)
```
...
```{r, fig.align='left', fig.width=8, fig.height=5, message=FALSE, warning=FALSE, error=TRUE}
