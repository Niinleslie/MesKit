...
_**MesKit: A Tool Kit for Dissecting Cancer Evolution of Multi-region Tumor Biopsies through Somatic Alterations (Under revise)**_
...
* `LOH` - the LOH (loss of heterozygosity) of segments (Logical) **Optional**
...
#### Via Bioconductor
Installing MesKit successfully requires version of Bioconductor is 3.12 or greater.
...
For a single region/tumor, `mutCluster` function clusters mutations in one dimension by variant allele frequencies (VAFs) or cellular cancer frequencies (CCFs) based on a Gaussian finite mixture model (using mclust). IF a segmentation file is available, `mutCluster` will only focus on mutations within copy-number neutral and LOH-free regions when clusering VAFs, as copy number aberrations will alter the fraction of reads bearing mutations. This function can produce a density distribution plot of VAFs or CCFs, depicting different clusters of mutations. Users can consider the number of clusters as a simple indicator of ITH. Note that, "low VAF/CCF" populations might be a mixture of subclones mutations represent admixed polyphyletic lineages [14](#refer).
...
14. Caravagna, G., Heide, T., Williams, M.J. et al. Subclonal reconstruction of tumors by using machine learning and population genetics. Nat Genet 52, 898–907 (2020). [https://doi.org/10.1038/s41588-020-0675-5](https://doi.org/10.1038/s41588-020-0675-5)
