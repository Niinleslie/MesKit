...
## Maf objects
`readMaf` function reads MAF files along with specific sample information, stores it as a MAF object. The Maf class inherits from matools::MAF. We also recommend users provide ccf data generated by PyClone for clone analysis. Additionally, parameter `refBuild` is used to set particular full genome sequences for Homo sapiens(BSgenome.Hsapiens.UCSC) referece for your MAF data.
