...
#'                 refBuild = "hg19")
#' mutPrivateShared(maf)
#' @export mutPrivateShared

mutPrivateShared <- function(maf, show.num = FALSE){
    maf.dat <- maf@data
    patient.ids <- unique(maf.dat$Patient_ID)
    msp.list <- maf.dat %>% group_by(Patient_ID) %>% 
                group_map(~doMutPrivateShared(.x,show.num = show.num), keep = TRUE) %>%
                rlang::set_names(patient.ids)
    return(msp.list)
}

doMutPrivateShared <- function(df, show.num){
