#'  plotMutSigProfile
#' 
#' @param sig_input result generated by function fitSignatures or triMatrix.
...
plotMutSigProfile <- function(sig_input, patient.id = NULL, mode = NULL){
    ...
    if(class(sig_input[[1]]) == "matrix"){
        
        for(i in length(sig_input)){
    ...
    }else if(class(sig_input[[1]]) == "list"){
        if(!is.null(mode)){
