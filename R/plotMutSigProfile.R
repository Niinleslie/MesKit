...
#' @param use.tumorLabel Logical (Default: FALSE). Rename the 'Tumor_Sample_Barcode' with 'Tumor_Sample_Label'.
...
plotMutSigProfile <- function(sig_input, patient.id = NULL, mode = NULL, use.tumorLabel = FALSE){
    ...
    if(is(sig_input[[1]], "matrix") | all(is(sig_input[[1]], "list") & length(sig_input[[1]]) <=2)){
        ...
        processMSP1 <- function(i){
            ...
            if(use.tumorLabel){
                if(nrow(tsb.label) == 0){
                    stop("Tumor_Sample_Label was not found. Please check clinical data or let use.tumorLabel be 'FALSE'")
                }
        ...
        processMSP2 <- function(i){
            ...
            if(use.tumorLabel){
                if(!"tsb.label" %in% names(sig_input[[i]])){
                    stop("There is no information about the Tumor_Sample_Label. Please check clinical data in readMaf or let use.tumorLabel be FALSE")
                }
                tsb.label <- sig_input[[i]]$tsb.label
                tsb_tl <- tsb.label$Tumor_Sample_Label
                names(tsb_tl) <- tsb.label$Tumor_Sample_Barcode
