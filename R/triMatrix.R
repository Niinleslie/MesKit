...
triMatrix <- function(phyloTree,
  ...
  for(phyloTree in phyloTree_list){
    ...
    origin_context <- Biostrings::getSeq(get(refBuild),Rle(paste("chr",mut_branches$chr,sep = "")),mut_branches$pos-1, mut_branches$pos+1) 
    origin_context <- as.character(origin_context)
    context <- ref64[origin_context]
    
    mut_types <- paste(mut_branches$ref, mut_branches$alt, sep = ">")
    ...
    if(withinTumor){
      branch_data_list <- split(mut_branches, mut_branches$Branch_Tumor_ID)
    }else{
