...
#' @param math_vafRange set the VAF threshold for MATH Score calculation. 
#' The first number in the list given would be the minvaf parameter for mathScore 
#' and the second one would be the maxvaf parameter. Default c(0, 1).
...
vafCluster <-function(maf, plotOption="ridges", themeOption="aaas", 
                      fileFormat=NULL, math_vafRange=c(0,1), vafColumn="VAF", 
                      outputDir=NULL){
    ...
    if ((plotOption == "allSeparate") | (plotOption == "allCombined")){
        ...
        for (counterMt in seq_along(tsbLs[,1])){
            ...
            ## calculate ScoreMATH
            mathscore <- .mathCal(maf, math_vafRange, 
    ...
    else 
    {
        ...
        colnames(clusterMt)[colnames(clusterMt) == "t_vaf"] <- "VAF"
        ## calculate ScoreMATH
        mathscore <- .mathCal(maf, math_vafRange, plotOption)
...
## Calculate ScoreMATH
.mathCal <- function(maf, math_vafRange,  
                     plotOption, sampleName = ""){
    if (!is.null(math_vafRange)){
        if ((plotOption == "allSeparate") | (plotOption == "allCombined")){
            mathscore <- mathScore(maf, c(sampleName), minvaf=math_vafRange[1], maxvaf=math_vafRange[2])
            ...
            mathscore <- mathScore(maf, c(plotOption), minvaf=math_vafRange[1], maxvaf=math_vafRange[2])
            mathscore <- mathscore[which(
