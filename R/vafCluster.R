...
#' @param vafRange set the VAF threshold for MATH Score calculation. 
...
vafCluster <-function(maf, vafRange=c(0,1), vafColumn="VAF", 
                      plotOption="compare", themeOption="aaas", 
                      fileFormat=NULL, outputDir=NULL){
...
## Calculate ScoreMATH
.mathCal <- function(maf, vafRange,  
                     plotOption, sampleName = ""){
    if (!is.null(vafRange)){
        if ((plotOption == "separate") | (plotOption == "combine")){
            mathscore <- mathScore(maf, c(sampleName), minvaf=vafRange[1], maxvaf=vafRange[2])
            ...
            mathscore <- mathScore(maf, c(plotOption), minvaf=vafRange[1], maxvaf=vafRange[2])
            mathscore <- mathscore[which(
