...
VAF_plot <-function(maf, sample_option = "OFA", theme_option = "aaas", file_format = "png", show.MATH = T)
{
  ...
  laml <- maf
  # extract vaf info
  samples <- data.frame(maf_input$Tumor_Sample_Barcode)
  cluster_all <- data.frame()
  n <- length(maf_input[,1])
  vaf_input_mt <- data.frame(maf_input$Hugo_Symbol, maf_input$VAF, samples)
  ...
  for (counter_mt in 1:length(tsb_ls[,1]))
    ...
    for (sample_name_mt in tsb_ls)
      ...
      if (show.MATH){
        MATH.score <- MATH_score(maf, c(as.character(sample_name_mt)[counter_mt]))
  ...
  } else if (sample_option == "OFA")
  {
    ...
    for (counter_mt in 1:length(tsb_ls[,1]))
      ...
      for (sample_name_mt in tsb_ls)
        ...
        # calculate MATH_score
        if (show.MATH){
          MATH.score <- MATH_score(maf, c(sample_name_mt))
  ...
  # specific sample
    # calculate MATH_score
    if (show.MATH){
      MATH.score <- MATH_score(maf, c(sample_option))
