...
mutSharedPrivate <- function(maf, show.num = FALSE){
  ...
  maf.dat <- maf@data[,1:15]
  
  patientID <- maf@patientID

  analysis <- function(df = NULL, patientID = patientID ,show.num = FALSE){
    ...
    for (n in 1:length(df$Hugo_Symbol)){
      ...
      new_gene <- c(new_gene,na.omit(strsplit(df$Hugo_Symbol[n],split = ",")[[1]][-1]))
    ...
    for (n in 1:newGeneNumber){
      ...
      df[primitiveLength+n,1] <- new_gene[n]
    ...
    getIntersect <- function(sample,information){
      ...
      while (n>0 & n<combination_number+1){
        ...
        while (a<sample_number+1){
          ...
          to_be_intersected_n <- c(to_be_intersected_n,list(information$neededData[which (information$Tumor_Sample_Barcode==sample[,n][a])]))
        ...
        allIntersect <<- c(allIntersect,list(length(result_n)))
      
        allIntersect <<- c(allIntersect,list(result_n))
    ...
    process <- lapply(sample_combination,getIntersect,df)
    ...
    allIntersect_only <- list()
    
    while(o>0 & o<length(allIntersect)+1){
    
      if (o%%2==0){
    
        allIntersect_only <- c(allIntersect_only,list(allIntersect[o][[1]]))
    ...
    while(n>0 & n<sample_number+1){
      ...
      seperate_n <- setdiff(unlist(allIntersect_only[n]),unlist(allIntersect_only[sub_n]))
      ...
      while (a>0 & a<length(seperate_n)+1){
        ...
        final_seperate_n <- c(final_seperate_n,as.character(unique(df$Variant_Classification[which (df$neededData==seperate_n[a])])))
    ...
    while(q>1 & q<sample_number+1){
      ...
      while(p>sum_CNM(sample_number,q-1) & p<sum_CNM(sample_number,q)+1){
        ...
        while (n>0 & n<(length(allIntersect_only[p][[1]])+1)){
    
          types_n <- c(types_n,as.character(unique(df$Variant_Classification[which (df$neededData==allIntersect_only[p][[1]][n])])))
    ...
    all_final_frame$Type  <-  factor(all_final_frame$Type, levels =unique(all_final_frame$Type))
    ...
    point_line_plot  <-  ggplot_gtable(ggplot_build(point_line_plot))
    
    bar_plot  <-  ggplot_gtable(ggplot_build(key_point)) 
    
    point_line_plot$widths  <-  bar_plot$widths
    ...
    if(length(levels(all_final_frame$Sample)) < 21){
      ...
      gg <- ggdraw()+draw_plot(bar_plot,0,0.3,1,0.7)+draw_plot(point_line_plot,0,0,1,0.35)
  ...
  analysis(maf.dat, patientID = patientID, show.num = show.num)
