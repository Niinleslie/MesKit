...
plotPhyloTree <- function(phyloTree = NULL,
   ...
   if(!is.null(branchCol)){
       ...
       if(!branchCol %in% branchCol.options){
           stop("Error:branchCol can only be 'mutSig','mutType' or NULL")
       }
   }
   if(class(phyloTree) == "phyloTree"){
       ...
       stop("Error:phyloTree should be class phyloTree or class phyloTreeList object generated by function getPhyloTree")
   }
   ...
   for(phyloTree in phyloTree_list){
       ...
       if(!is.null(phyloTree@tree$data)){
           ...
           common.col <- phyloTree@tree$common.col
           compare <- TRUE
       }else{
           compare <- FALSE
