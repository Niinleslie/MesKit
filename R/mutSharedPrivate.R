...
mutSharedPrivate <- function(maf, show.num = FALSE){
  ...
  while (n>0 & n<primitiveLength + 1){
    ...
    if (geneNumber > 1){
      ...
      while (geneNumber>1){
        ...
        df <- rbind(df,newRow) 
        df[l+1,] <- df[n,]
        df$Hugo_Symbol[l+1] <- as.character(strsplit(df$Hugo_Symbol[n],split = ",")[[1]][geneNumber])
  ...
  for (n in 1:length(df$Hugo_Symbol)){
    if (length(strsplit(as.character(df$Variant_Classification[n]),split = "\t")[[1]])>2){
      df[n, ] <- NA
    }
  }
  df <- dplyr::select(df,c(Hugo_Symbol,Chromosome,Start_Position,End_Position,Variant_Classification, Reference_Allele,Tumor_Seq_Allele1,Tumor_Seq_Allele2,Tumor_Sample_Barcode))
  df <- na.omit(df)
  #organize dataframe
