...
TumorClones_plot.shiny <- function(patientID, ccf.cluster.dir = "../data/", ccf.loci.dir = "../data/", out.dir = "./Figures/", clone.min.mut = 5, clone.min.aveCCF = 0.1){
  ccf <- read.table(ccf.cluster.dir , sep = "\t", header = T, stringsAsFactors = F)
  ccf <- ccf[which(ccf$size>=5 & ccf$mean>=0.1),c(1,2,4)]
  cluster.ids <- unique(ccf$cluster_id)
  
  loci <- read.table(ccf.loci.dir, sep = "\t", header = T, stringsAsFactors = F)
