...
#' @description Read maf file as data.frame. Define branches' 
#' set relationship by re-labeling their tumor sample 
#' barcode from the smallest set. Calcualte each branch's 
#' mutational signature weight according to cosmic reference
#' and pick the maxium. Return a data frame of each set/branch's 
#' mutational signature.
#' 
#' @import reshape2 BSgenome BSgenome.Hsapiens.UCSC.hg19 GenomeInfoDb 
#' grDevices graphics utils deconstructSigs
#' @import plyr
#' 
#' @param maf_file specify a maf document/directory as the input of 
#' the function
#' @param branch_file specify a txt document/directory as the input 
#' of the branches (needed to be refined)
#' @return data frame of each set/branch's mutational signature.
#'
#' @examples
#' \dontrun{
#' mutationalSigsTree(maf_file, branch_file)
#' mutationalSigsTree(maf_file, branch_file, driver_genes_dir)
#' mutationalSigsTree(maf_file, branch_file, driver_genes_dir, 
#' mut.threshold=30)
#'}
...
mutationalSigsBranch <- function(mut.sig.ref, mut.sigs.output, 
                                   ...
                                   ref.build){
    if (length(mut.sig.ref[which(
        mut.sig.ref$Sample == branch_name), 1]) < mut.threshold){
    ...
    if (typeof(driver_genes_dir) == "character"){
        ...
        mut.sigs.branch <- data.frame(
            ...
            sig=sigs.max.name, 
            mut.num=length(
                mut.sig.ref[which(mut.sig.ref$Sample == branch_name), 1]), 
            sig.prob=sigs.max.prob, putative_driver_genes=I(
                list(pdg.branch)))
    } else{
        mut.sigs.branch <- data.frame(
            branch=I(list(branch)), 
            sig=sigs.max.name, 
            mut.num=length(mut.sig.ref[which(
