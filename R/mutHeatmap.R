...
#'
#' @param maf Maf or Maf list object generated by readMaf function
...
mutHeatmap <- function(maf,
    ...
    if(class(maf) == "classMaf"){
        ...
        if(!is.null(patient.id)){
            maf <- subsetMaf_list(maf,
                                  patient.id = patient.id)
        }
        maf_list <- maf@patient.list
    }else{
        stop("maf should be either maf or maf list")
    }
    ...
    for(m in maf_list){
        maf_data <- subsetMaf(m,
                       ...
                       mutType = mutType,
                       use.indel = use.indel) %>% 
            do.classify(class = "SP",classByType = T) %>% 
