...
mutHeatmap <- function(maf,
    ...
    for(i in 1:length(mafData_list)){
        ...
        patient.data <- mafData_list[[i]]
        patient <- unique(patient.data$Patient_ID)
        
        ## get mutation matrix
        mut_sort <- getMutMatrix(patient.data, use.ccf = FALSE)
        if("CCF" %in% colnames(patient.data)){
            ccf_sort <- getMutMatrix(patient.data, use.ccf = TRUE)
        }else{
            ccf_sort <- matrix() 
        }
        ...
        class_all_colors <- c( "Public" = "#7fc97f",
                               "Private" = "#E64B35FF",
                               "Shared" = "#fdc086")
        class_colors <- c()
        class_name_percentage <- c()
        for(class in classes.level){
            class_colors <- append(class_colors, class_all_colors[as.character(class)])
            ## percentage of class
            ...
            name_percentage <- paste0(as.character(class),paste0("(",round(class.num,3)*100,"%)"))
            class_name_percentage <- append(class_name_percentage, name_percentage)
        ...
        names(class_colors) <- class_name_percentage
        # class_colors <- as.factor(class_colors)
        colnames(annotation.bar) <- c("xmin","xmax","ymin","ymax", "class")
...
#     ggplot()+
#         geom_rect(data = annotation.bar,
#                   mapping = aes(xmin = xmin,xmax = xmax,ymin = ymin, ymax = ymax, fill = factor(class) )) +
#         scale_fill_manual(values = class_colors,name = "Class"))%>% 
#     ggplotGrob %>% 
#     {.$grobs[[which(sapply(.$grobs, function(x) {x$name}) == "guide-box")]]}
#     ggplotGrob %>%
#     {.$grobs[[which(sapply(.$grobs, function(x) {x$name}) == "guide-box")]]}
# heat_map <- ggdraw(p+theme(legend.position = "none")) + 
#     draw_plot(mutation_legend, x = 1, y = 0.2)
# multi legends
# legends_column <-
#     plot_grid(
