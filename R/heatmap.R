...
#'
#' @examples
#' mut.heatmap(phyloTree)
#' mut.heatmap(phyloTree, use.ccf = TRUE)
#'
#' @export mut.heatmap
...
mut.heatmap <- function(phyloTree, use.ccf = FALSE){
  ...
  if(use.ccf){
    ...
    mat <- ccf_sort
    if(is.null(ccf)){
      stop("phyloTree@ccf_sort is NULL. No ccf data was found when readMaf")
    }
  }
