#' Stack Plot for variatns of oncogenes and Tumor Suppression Gene(TSGs) in patient
#' @description With the gene list passed by oncogeneListFile and tsgListFile, 
#' mutStackPlot can calculate the quantity of variants from different 
#' intersections and summarize the result as a stack plot. The stacks will be 
#' constructed by three part: 1. Private: variants specific for each samples;
#' 2. Parital-shared: variants shared by multiple samples but not existing in 
#' all samples; 3. Shared: variants shared by all samples.
#' 
#' @import ggplot2 ggsci
#' 
#' @param maf the classMaf object generated by readMaf.
#' @param oncogeneListFile the file containing all oncogenes needed to be checked.  
#' @param tsgListFile the file containing all TSGs needed to be checked. 
...
#' mutStackPlot(maf, oncogeneListFile, tsgListFile, show.percentage=TRUE)
#' 
#' @export mutStackPlot
