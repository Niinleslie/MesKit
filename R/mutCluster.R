#' mutCluster
#' @description Cluster mutatinos by CCFs or VAFs (if segment file is provided, only mutations in copy-number
#' neutral regions will be included. portions of the genome)
#'  
...
#' @return clustering plots of ccfs or vafs
#' @export mutCluster
  ...
  processVafcluster_maf <- function(m){
    ...
    if(withinTumor){
      if(!"CCF" %in% colnames(getMafData(m))){
        stop(paste0("mutCluster requires CCF data when 'withinTumor' is TRUE." ,
      ...
      if(use.ccf){
        if(!"CCF" %in% colnames(getMafData(m))){
          stop(paste0("mutCluster requires CCF data when 'use.CCF' is TRUE." ,
                      "No CCF data was found when generating Maf/MafList object."))
        }
        ...
        if(!is.null(segFile)){
          ...
          message("No segment files specified. Assuming all mutations have a copy number of 2.")
        }
